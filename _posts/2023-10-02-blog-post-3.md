...
- 1. First we need to load our data.
-  2. Text splitters break Documents into splits of specified size.
- 3. Storage (e.g., often a vectorstore) will house and often embed the splits.
- 4. The app retrieves splits from storage (e.g., often with similar embeddings to the input question).
- 5. An LLM produces an answer using a prompt that includes the question and the retrieved data.
