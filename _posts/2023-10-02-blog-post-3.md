...
In-Context Learning (ICL) represents a distinct approach to prompt engineering, wherein task demonstrations are integrated into the prompt in natural language format. With ICL, the capability of LLMs is harnessed to tackle novel tasks without fine-tuning. Moreover, ICL offers the flexibility of being combined with fine-tuning, unlocking even more significant potential for more robust and dynamic LLMs. The ICL can be divided into zer-shot and few-shot prompting.
...
As it can be seen in Figure 3, the prompt lacks an involving solved example, leading us to characterize this prompting approach as *zero-shot prompting*.
