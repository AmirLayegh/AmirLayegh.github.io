...
$$H_2 = Self-Attn(H_1)$$, where $H_2$ is node embeddings after calculating self-attention. The second step involves transformation of input tokens $X$ embedding to the space of node mebeddings $H_2$:

$$T^ \prime = FFN_1(\sigma(FFN_2(T)))$$, where $T^ \prime$ indicates the embedding of input tokens in the dimension of node embeddings, and T is the input tokens representation. $\sigma$ is the GELU actication function and $FFN_1$ and $FFN_2$ are feed-forward networks. Afterwards, we calculate the cross-modality attention using the $H_2$ as query and $T^ \prime$ as key and value:

$$H_3 = softmax[H_2.(T^ \prime)^{\mathcal{T}} / \sqrt(d_g)].T^ \prime$$, where $H_3$ is the final node embeddings obtained with cross-modality attention. Finally, we generate the graph-level embedding by average pooling the node embeddings $H_3$:
